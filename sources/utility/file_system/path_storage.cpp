...
util::PathStorage::addFileNonstatic(std::string_view a_file_name,
                                    std::string_view a_file_path,
                                    const core::Context* a_context) noexcept
{
    return addObject(m_files_paths, a_file_name, a_file_path, a_context, false);
}
...
util::PathStorage::addFolderNonstatic(std::string_view a_folder_name,
                                      ...
                                      const core::Context* a_context) noexcept
{
    return addObject(m_folder_paths, a_folder_name, a_folder_path, a_context,
                     true);
}
...
util::PathStorage::addFolderNonstatic(std::string_view a_folder_path,
                                      const core::Context* a_context) noexcept
{
    return addObject(m_files_paths, Path::getName(a_folder_path, *a_context),
                     a_folder_path, a_context, true);
}
    ...
    bool a_is_folder) noexcept
{
    ...
    if (it != a_storage.end())
    {
        auto& new_element_it =
            *m_str_storage
                 .insert(a_is_folder ? Path::normalizeFolderPath(a_name)
                                     : std::string(a_name))
                 .first;
                                        ...
                                        const core::Context* a_context) noexcept
{
    ...
    if (!path.has_value())
    {
        result = Path::combine(path.value(), a_file_name);
    }
    return result;
}
...
util::PathStorage::touchFolderNonstatic(std::string_view a_folder_name,
                                        ...
                                        const core::Context* a_context) noexcept
{
    return touchFolderImpl(
        a_folder_name, Path::combine(a_parent_path, a_folder_name), a_context);
}
    ...
    const core::Context* a_context = nullptr) noexcept
{
    ...
    auto stored_folder_opt = getFolderPath(a_folder_name);
